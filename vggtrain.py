...
def train(learn_rate, batch_size):
    ...
    for epoch in range(EPOCHS):
        ...
        paddle.save(model.state_dict(), f'./output/VGG/{learn_rate}/{batch_size}/model_{epoch}')
        if best_result < acc_mean:
            best_result = acc_mean
            best_epoch = epoch
            paddle.save(model.state_dict(), f'./output/VGG/{learn_rate}/{batch_size}/best_model')
